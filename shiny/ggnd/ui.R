    ...
    navset_hidden(
      ...
      nav_panel(
        ...
        layout_column_wrap(
          ...
          bslib::card(
            card_header("Sorting"),
            selectizeInput("in_sel_pcp_order_columns", label = "Order Columns By", choices = c("auto", "appearance", "random"), multiple = FALSE, selected = "auto"),
            selectInput("in_sel_pcp_order_observations_by", label = "Order observations by", choices = c("frequency", "original")),
            selectizeInput("in_sel_pcp_scaling", label = "Scaling", choices = c("uniminmax", "none"), multiple = FALSE, selected = "uniminmax")
          ),
          bslib::card(
            card_header("Colour"),
            selectInput("in_sel_pcp_col_colour", label = "Colour by", choices = ""),
            selectInput("in_sel_pcp_highlight", label = "Highlight", choices = ""),
            # selectInput("in_sel_pcp_col_colour", label = "Colour by", choices = NULL),
          ),
          bslib::card(
            card_header("Size"),
            # Interactivity size
            numericInput("in_pcp_svg_width", "Interactive SVG Width", value = 16, min = 1),
            numericInput("in_pcp_svg_height", "Interactive SVG Height", value = 5, min = 1)
          ),
        )
      )
    )
